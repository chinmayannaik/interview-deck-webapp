...
(function () {
  ...
  IQB.data.angular = [
    ...
    {
      id: "ng-switchmap-live-search",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "rxjs", "switchMap", "debounceTime", "api", "real-world"],
      question:
      ...
      answer:
        '<p>Use <code class="inline">debounceTime()</code> to wait until the user stops typing or clicking for a short duration, reducing unnecessary API calls. Then use <code class="inline">switchMap()</code> to automatically cancel any previous pending request whenever a new request is triggered. This ensures that only the latest response is processed and displayed.</p>',
      tip: "Remember: <strong>debounceTime delays the request, switchMap cancels the previous request.</strong>",
      code:
        "this.search$.pipe(\n" +
        "  debounceTime(300),\n" +
      ...
      deep:
        ...
        "<p>Imagine an AI dashboard where the user clicks different timestamps on a video to request AI analysis. Each click triggers an HTTP request.</p>" +
        "<p>If the user clicks Timestamp A, then immediately clicks Timestamp B, you don't want the response from A arriving after B and overwriting the latest result.</p>" +
        "<p><strong>switchMap()</strong> automatically unsubscribes from (cancels) the previous HTTP request and subscribes only to the latest one, ensuring stale data is ignored.</p>" +
        "<h4>Role of debounceTime()</h4>" +
        '<p><code class="inline">debounceTime(300)</code> waits until the user pauses for 300ms before sending the request. This prevents flooding the server with unnecessary API calls during rapid typing or clicking.</p>' +
        "<h4>Why switchMap?</h4>" +
        "<table><thead><tr><th>Operator</th><th>Behavior</th></tr></thead><tbody>" +
        "<tr><td>switchMap</td><td>Cancels previous request and keeps only the latest.</td></tr>" +
        "<tr><td>mergeMap</td><td>Runs all requests concurrently.</td></tr>" +
        "<tr><td>concatMap</td><td>Queues requests and executes one after another.</td></tr>" +
        "<tr><td>exhaustMap</td><td>Ignores new requests until the current one finishes.</td></tr>" +
        "</tbody></table>" +
        "<h4>How to answer in an interview</h4>" +
        '<p>“For rapid user interactions like live search or AI analysis, I\'d combine <code class="inline">debounceTime()</code> and <code class="inline">switchMap()</code>. debounceTime reduces unnecessary API calls, while switchMap automatically cancels previous requests so only the latest response updates the UI. This prevents race conditions and keeps the frontend responsive.”</p>',
    },
    {
      id: "ng-large-list-performance",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "performance", "trackBy", "virtual-scroll", "real-world"],
      question:
        "Your page displays 10,000 records and scrolling becomes very slow. How would you optimize it?",
      answer:
        '<p>First, use <code class="inline">trackBy</code> so Angular reuses existing DOM elements instead of recreating them. For very large datasets, use <strong>CDK Virtual Scroll</strong> so only the visible items are rendered. Combine this with <code class="inline">ChangeDetectionStrategy.OnPush</code> to reduce unnecessary change detection.</p>',
      tip: "The three keywords interviewers expect are: <strong>trackBy</strong>, <strong>Virtual Scroll</strong>, and <strong>OnPush</strong>.",
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>A dashboard loads thousands of customer records. Without optimization Angular creates thousands of DOM elements, making scrolling slow.</p>" +
        "<h4>Solution</h4>" +
        "<ul>" +
        "<li>Use <strong>trackBy</strong> to reuse DOM elements.</li>" +
        "<li>Use <strong>CDK Virtual Scroll</strong> to render only visible rows.</li>" +
        "<li>Use <strong>OnPush</strong> to reduce change detection.</li>" +
        "</ul>" +
        "<h4>Interview answer</h4>" +
        '<p>"I first reduce DOM creation using trackBy. If the dataset is huge, I implement Angular CDK Virtual Scroll so only visible rows exist in the DOM. Finally, I use OnPush to minimize unnecessary change detection."</p>',
    },
    {
      id: "ng-memory-leak-subscriptions",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "rxjs", "subscriptions", "memory-leak", "real-world"],
      question: "How do you prevent memory leaks caused by RxJS subscriptions?",
      answer:
        '<p>Always unsubscribe from long-lived Observables when the component is destroyed. In modern Angular, the recommended approach is using <code class="inline">takeUntilDestroyed()</code>. In older versions, use <code class="inline">takeUntil()</code> with a Subject or rely on the <code class="inline">async</code> pipe whenever possible.</p>',
      tip: "The Async Pipe automatically subscribes and unsubscribes for you.",
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>A component subscribes to WebSocket updates. Every time the user navigates away and back, another subscription is created, causing duplicate events and increasing memory usage.</p>" +
        "<h4>Best practices</h4>" +
        "<ul>" +
        "<li>Prefer the Async Pipe in templates.</li>" +
        '<li>Use <code class="inline">takeUntilDestroyed()</code> for manual subscriptions.</li>' +
        "<li>Avoid storing unnecessary subscriptions.</li>" +
        "</ul>" +
        "<h4>Interview answer</h4>" +
        '<p>"I avoid memory leaks by preferring the Async Pipe. For manual subscriptions I use takeUntilDestroyed(), ensuring Angular automatically cleans them up when the component is destroyed."</p>',
    },
    {
      id: "ng-prevent-double-submit",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "rxjs", "forms", "exhaustMap", "real-world"],
      question:
        "How would you prevent users from submitting the same form multiple times by clicking the Save button repeatedly?",
      answer:
        '<p>Disable the Save button while the request is in progress or use <code class="inline">exhaustMap()</code>. It ignores subsequent button clicks until the current request completes, preventing duplicate submissions.</p>',
      tip: "Remember: <strong>switchMap cancels previous</strong>, <strong>exhaustMap ignores new requests</strong>.",
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>A user clicks Save five times because the API is slow. Without protection, five identical orders may be created.</p>" +
        "<h4>Solution</h4>" +
        "<ul>" +
        "<li>Disable the Save button.</li>" +
        "<li>Or use <strong>exhaustMap()</strong> so additional clicks are ignored until the first request completes.</li>" +
        "</ul>" +
        "<h4>Interview answer</h4>" +
        '<p>"For Save buttons I usually combine a loading state with exhaustMap(), ensuring duplicate submissions are ignored while the current request is running."</p>',
    },
    {
      id: "ng-component-communication",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "component-communication", "real-world", "architecture"],
      question:
        "How would you share data between unrelated Angular components?",
      answer:
        '<p>For unrelated components, avoid directly referencing each other. Use a shared service with an RxJS <code class="inline">BehaviorSubject</code> or use NgRx if the state is shared across multiple features. Parent-child communication should still use <code class="inline">@Input()</code> and <code class="inline">@Output()</code>.</p>',
      tip: "The communication mechanism depends on the relationship between components.",
      code:
      ...
      deep:
        ...
        "<tr><td>Parent → Child</td><td>@Input()</td></tr>" +
        "<tr><td>Child → Parent</td><td>@Output()</td></tr>" +
        "<tr><td>Unrelated Components</td><td>Shared Service + BehaviorSubject</td></tr>" +
        "<tr><td>Large Application</td><td>NgRx Store</td></tr>" +
        "</tbody></table>" +
        "<h4>Interview answer</h4>" +
        '<p>"I choose the communication mechanism based on the relationship between components. Parent-child uses Input/Output, unrelated components use a shared service, and application-wide shared state uses NgRx."</p>',
    },
    {
      id: "ng-lazy-loading-performance",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "lazy-loading", "routing", "performance", "real-world"],
      question:
      ...
      answer:
        "<p>I would split the application into feature modules and lazy load them using the Angular Router. This ensures only the modules required for the current route are downloaded initially, reducing the bundle size and improving the application's startup time.</p>",
      tip: "The first optimization interviewers expect is <strong>Lazy Loading</strong>.",
      code:
        "const routes: Routes = [\n" +
        "  {\n" +
        "    path: 'admin',\n" +
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>Your application has 15 modules, but users usually access only 2 or 3 of them. Loading everything during startup increases the bundle size and slows down the application.</p>" +
        "<h4>Solution</h4>" +
        "<ul>" +
        "<li>Use Lazy Loading for feature modules.</li>" +
        "<li>Preload frequently used modules if necessary.</li>" +
        "<li>Keep the initial bundle as small as possible.</li>" +
        "</ul>" +
        "<h4>Interview answer</h4>" +
        '<p>"I split the application into feature modules and lazy load them using the router. This reduces the initial bundle size, improves startup time, and downloads features only when users navigate to them."</p>',
    },
    {
      id: "ng-api-caching",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "http", "rxjs", "cache", "shareReplay", "real-world"],
      question:
        "Multiple components call the same API, causing duplicate network requests. How would you optimize it?",
      answer:
        '<p>I would cache the API response inside a shared service using <code class="inline">shareReplay(1)</code>. The first component triggers the HTTP request, while subsequent subscribers receive the cached response without making additional network calls.</p>',
      tip: '<code class="inline">shareReplay(1)</code> is the most common RxJS operator used for HTTP caching.',
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>The Header, Dashboard and Profile components all request the logged-in user. Without caching, three identical HTTP requests are sent.</p>" +
        "<h4>Solution</h4>" +
        '<p>Store the Observable in a service and use <code class="inline">shareReplay(1)</code> so every component receives the same cached result.</p>' +
        "<h4>Interview answer</h4>" +
        '<p>"I\'d cache the HTTP Observable using shareReplay(1), ensuring the API is called only once while all components share the same response."</p>',
    },
    {
      id: "ng-route-guard-auth",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "routing", "guards", "authentication", "real-world"],
      question:
        "How would you prevent unauthenticated users from accessing protected pages?",
      answer:
        '<p>I would use an Angular Route Guard such as <code class="inline">CanActivate</code>. Before navigation, the guard checks whether the user is authenticated. If not, it redirects the user to the login page.</p>',
      tip: "Authentication checks belong in Route Guards, not inside every component.",
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>A user manually enters the URL '/admin'. Even though the menu is hidden, they should still be prevented from accessing the page.</p>" +
        "<h4>Solution</h4>" +
        "<p>Use a Route Guard to verify authentication before navigation.</p>" +
        "<h4>Interview answer</h4>" +
        '<p>"Instead of checking authentication inside every component, I centralize the logic using a Route Guard, ensuring protected routes cannot be accessed directly."</p>',
    },
    {
      id: "ng-onpush-real-world",
      category: "angular",
      difficulty: "advanced",
      tags: [
        "V.Imp",
      ...
      answer:
        '<p>I would use <code class="inline">ChangeDetectionStrategy.OnPush</code>. With OnPush, Angular checks the component only when an <code class="inline">@Input()</code> reference changes, an event occurs, or an Observable emits. This significantly reduces unnecessary change detection cycles.</p>',
      tip: "OnPush works best with immutable objects and the Async Pipe.",
      code:
        "@Component({\n" +
        "  selector: 'app-users',\n" +
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>A dashboard contains many widgets that refresh frequently. Default change detection causes every component to be checked after each event, reducing performance.</p>" +
        "<h4>Solution</h4>" +
        "<ul>" +
        "<li>Use OnPush.</li>" +
        "<li>Avoid mutating objects directly.</li>" +
        "<li>Prefer immutable updates and the Async Pipe.</li>" +
        "</ul>" +
        "<h4>Interview answer</h4>" +
        '<p>"I\'d switch the component to OnPush so Angular performs change detection only when necessary. Combined with immutable data and the Async Pipe, this greatly improves rendering performance."</p>',
    },
    {
      id: "ng-auto-save-form",
      category: "angular",
      difficulty: "advanced",
      tags: [
        "V.Imp",
      ...
      answer:
        '<p>I would subscribe to <code class="inline">form.valueChanges</code>, apply <code class="inline">debounceTime()</code> to wait until the user stops typing, use <code class="inline">distinctUntilChanged()</code> to avoid duplicate requests, and finally use <code class="inline">switchMap()</code> to cancel any pending save request when new changes occur.</p>',
      tip: "Auto-save is one of the best real-world examples combining <strong>debounceTime</strong>, <strong>distinctUntilChanged</strong> and <strong>switchMap</strong>.",
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>While writing a document or filling a long form, users expect their work to be saved automatically without clicking a Save button.</p>" +
        "<h4>Solution</h4>" +
        "<ul>" +
        '<li>Listen to <code class="inline">valueChanges</code>.</li>' +
        "<li>Wait until the user pauses typing.</li>" +
        "<li>Ignore duplicate values.</li>" +
        "<li>Cancel previous save requests if new edits occur.</li>" +
        "</ul>" +
        "<h4>Interview answer</h4>" +
        '<p>"I\'d use valueChanges with debounceTime(), distinctUntilChanged() and switchMap() so drafts are saved efficiently without flooding the server."</p>',
    },
    {
      id: "ng-global-error-handling",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "http", "interceptor", "error-handling", "real-world"],
      question:
        "How would you handle HTTP errors consistently across the entire Angular application?",
      answer:
        "<p>I would create an <strong>HTTP Interceptor</strong>. It centralizes error handling, logging, authentication failures, and token refresh logic instead of duplicating the same code inside every component or service.</p>",
      tip: "Cross-cutting concerns like authentication and error handling belong in an Interceptor.",
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>Every API call should display consistent error messages and automatically redirect users when a 401 Unauthorized response is received.</p>" +
        "<h4>Interview answer</h4>" +
        '<p>"Instead of handling errors in every component, I\'d centralize them using an HTTP Interceptor. This keeps the code cleaner and ensures consistent behavior throughout the application."</p>',
    },
    {
      id: "ng-token-refresh",
      category: "angular",
      difficulty: "advanced",
      tags: ["V.Imp", "authentication", "jwt", "interceptor", "real-world"],
      question:
        "How would you handle an expired JWT token without forcing the user to log in again?",
      answer:
        '<p>I would detect a <code class="inline">401 Unauthorized</code> response inside an HTTP Interceptor, call the Refresh Token API, store the new access token, retry the original request, and only redirect to login if the refresh request also fails.</p>',
      tip: "The retry logic should be implemented inside an HTTP Interceptor, not in every API call.",
      code: "401 Error\n↓\nRefresh Token API\n↓\nNew Access Token\n↓\nRetry Original Request",
      lang: "",
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>A user's access token expires after one hour, but the refresh token is still valid. The user should continue working without interruption.</p>" +
        "<h4>Interview answer</h4>" +
        '<p>"I\'d intercept 401 responses, refresh the token transparently, retry the failed request, and only redirect to login if refreshing the token also fails."</p>',
    },
    {
      id: "ng-file-upload-progress",
      category: "angular",
      difficulty: "intermediate",
      tags: ["V.Imp", "http", "file-upload", "progress", "real-world"],
      question:
        "How would you display upload progress while uploading a large file?",
      answer:
        '<p>Angular\'s <code class="inline">HttpClient</code> supports upload progress events. By enabling <code class="inline">reportProgress</code> and <code class="inline">observe: \'events\'</code>, I can calculate the upload percentage and display a progress bar to the user.</p>',
      tip: "Always use upload progress for large files to improve user experience.",
      code:
      ...
      deep:
        "<h4>Real-world scenario</h4>" +
        "<p>A user uploads a 500 MB video. Without progress feedback, the application appears frozen and users may retry the upload.</p>" +
        "<h4>Interview answer</h4>" +
        "<p>\"I'd use HttpClient's progress events to calculate the upload percentage and display a progress bar, giving users clear feedback during long uploads.\"</p>",
    },
  ];
})();
